...
/**
 * 地址
 * https://blog.csdn.net/qq_39200794/article/details/118445798
 */
public final class GaussianBlur {
    ...
    public static void main(String[] args) throws Exception {
        BufferedImage bufferedImage = ImageIO.read(new File("C:\\Users\\dingzhixin.ex\\Desktop\\999.png"));
        BufferedImage image = gaussianBlur(bufferedImage);
        ImageIO.write(image, "png", new File("C:\\Users\\dingzhixin.ex\\Desktop\\8525.png"));
    }
...
//                    pixel = ((picture[0][i][j] & 0xff) << 16) + ((picture[1][i][j] & 0xff) << 8) + (picture[2][i][j] & 0xff);
                    pixel = ColorFilterUtil.argb(0xff, picture[0][i][j], picture[1][i][j], picture[2][i][j]);
                    //LUtils.i(Integer.toHexString(pixel));
    ...
    public static int[][][] GaussianBlur(int[][][] picture, int radius) {
        ...
        LUtils.i("matrix = ", Arrays.toString(matrix), " , ", matrix.length);
        ...
        for (i = 0; i < width; i++) {
            for (j = 0; j < height; j++) {
                ...
                for (x = j - radius; x <= j + radius; x++) {
                    ...
                    // TODO: 2022/7/22 对称点计算有问题
                    //subscript = (x >= 0 && x < height) ? x : 2 * j - x; // 如果坐标越界了，则计算对称点来代替
                    subscript = (x >= 0 && x < height) ? x : (x < 0 ? (Math.abs(height - j) % height) : (x - j) % height); // 如果坐标越界了，则计算对称点来代替
        ...
        for (i = 0; i < height; i++) {
            for (j = 0; j < width; j++) {
                ...
                for (x = j - radius; x <= j + radius; x++) {
                    ...
                    subscript = (x >= 0 && x < width) ? x : (x < 0 ? (Math.abs(height - j) % height) : (x - j) % height);
